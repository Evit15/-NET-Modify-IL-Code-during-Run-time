...
CString CSymbolAddressCache::s_strJitModuleMd5Hash;
CString CSymbolAddressCache::s_strClrModuleMd5Hash;


BOOL CSymbolAddressCache::TryCache(void)
{
	s_strJitModuleMd5Hash = CUtility::ComputeMd5HashForModule(g_hJitModule);
	s_strClrModuleMd5Hash = CUtility::ComputeMd5HashForModule(g_hClrModule);

	USES_CONVERSION;
	ULONG64 ulBaseAddr = (ULONG64)g_hJitModule;
	std::map<CString, ULONG64>::iterator iter;
	LPVOID * pTemp;
	std::map<CString, ULONG64> mpOffset = LoadCache(s_strJitModuleMd5Hash);
	LOAD_OFFSET(ICorJitCompiler::s_pfnComplieMethod);

	if( !ICorJitCompiler::s_pfnComplieMethod )
		return FALSE;

	ulBaseAddr = (ULONG64)g_hClrModule;
	mpOffset = LoadCache(s_strClrModuleMd5Hash);
...
void CSymbolAddressCache::GenerateJitCache(void)
{
	USES_CONVERSION;
	ULONG64 ulBaseAddr = (ULONG64)g_hJitModule;
	ULONG64 ulOffset = 0;
	...
	strFileName.AppendFormat( _T("%s.cache"), s_strJitModuleMd5Hash);

	CString strContent;
	strContent.Append(s_strJitModuleMd5Hash);
	SAVE_OFFSET(ICorJitCompiler::s_pfnComplieMethod);
...
void CSymbolAddressCache::GenerateClrCache(void)
{
	...
	strFileName.AppendFormat( _T("%s.cache"), s_strClrModuleMd5Hash);

	CString strContent;
	strContent.Append(s_strClrModuleMd5Hash);
